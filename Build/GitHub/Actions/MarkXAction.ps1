<#
.Synopsis
    GitHub Action for MarkX
.Description
    GitHub Action for MarkX.  This will:

    * Import MarkX
    * If `-Run` is provided, run that script
    * Otherwise, unless `-SkipScriptFile` is passed, run all *.MarkX.ps1 files beneath the workflow directory
...
param(
# A PowerShell Script that uses MarkX.  
# Any files outputted from the script will be added to the repository.
# If those files have a .Message attached to them, they will be committed with that message.
[string]
$Run,

# If set, will not process any files named *.MarkX.ps1
[switch]
$SkipScriptFile,
